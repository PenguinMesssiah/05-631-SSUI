...
class UIClass {
    ...
    handleClick(x : number, y : number) : boolean {
        ...
        let ifClickHandled: boolean =  false;
        for (let i = this.childObjects.length-1; i >= 0; i--) {
            if (this.childObjects[i].handleClickAt(x,y)) {
                ...
                //console.log("child handler returned true", this.childObjects[i])
                ifClickHandled = true;
            }
        }
        // nobody handled it so indicate that it was not handled
        return ifClickHandled;
    }
}
...
class ScreenObject {
    ...
    //set name(z : string) {this._name = z;}
...
class FittsTestUI extends UIClass {
    ...
    override configure(newState : string) {
        ...
        switch (this.currentState) {
            ...
            case 'begin_trial':
                // === YOUR CODE HERE ===
                this.theBackground.msg1 = "Trail #" + this.trialCount + " of 10"
                this.theBackground.msg2 = ""
                this.theBackground.msg3 = ""
                ...
                this.startTrial(this.theReticle.centerX, this.theReticle.centerY)
            break;
            case 'in_trial':
                // === YOUR CODE HERE ===
                this.recordTrialEnd(this.theTarget.centerX, this.theTarget.centerY, this.theTarget.diam)

                this.theBackground.msg1 = "Please Reset Cursor"
                this.theBackground.msg2 = ""
                this.theBackground.msg3 = ""
...
class Target extends ScreenObject{
    ...
    readonly TARGET_COLOR : string = 'lightsteelblue';
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        // === YOUR CODE HERE ===
        if((ptX <= this.centerX + this.radius/3 && ptX >= this.centerX - this.radius/3) && 
            (ptY <= this.centerY + this.radius/3 && ptY >= this.centerY - this.radius/3)) 
            return true;
        else
            return false;
...
class Reticle extends Target {
    ...
    // Color we are drawn with
    static readonly RETICLE_COLOR : string = 'palegreen';
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        if(this.parentUI.currentState === 'begin_trial') {
            this.parentUI.configure('in_trial')
        } else if (this.parentUI.currentState === 'in_trial') {
